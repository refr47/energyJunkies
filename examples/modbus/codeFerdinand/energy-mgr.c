...
// shut down modbus connection
    ...
    fprintf(stderr, "%s: Termination Time %d o'Clock reached. Good Bye.\n", timeNowText, config.lastHour);
    return 0;
} // main
...
void showDischargeHours(config_t *cfg, int month, float kWhAvail) {
    ...
    for (int i = 0; i < HOUR_COUNT /*&& dischargeRate[consTariff[i].index] > 0*/; i++) {
        if (cfg->dischargeRate[cfg->consTariff[i].index] > 0) {
            if (cfg->consTariff[i].index * 60 <= cfg->pvMinutes[month].begin ||
                cfg->consTariff[i].index * 60 >= cfg->pvMinutes[month].end) {
...
int findOrigIndex(config_t *cfg, int hour) {
    for (int i = 0; i < HOUR_COUNT; i++) {
        if (cfg->consTariff[i].index == hour) {
            return i;
        }
    }
    return -1;
...
void showDischargeHoursCron(config_t *cfg, int month, float kWhAvail) {
    printf("\nVAT%%\tDiChg%%\tGrid ct\tFeed ct\tAdd ct\tRed ct\tDiCh ct\tChrg ct\tBattEff\tAcc kWh\n%d\t%d\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\n",
           cfg->vatPercent, cfg->dischargePercent, cfg->gridTariff, cfg->feedTariff, cfg->feedTariffDischargePlus,
           cfg->feedTariffChargeMinus, cfg->dischargeThresholdNet, cfg->chargeThresholdNet,
           cfg->batteryEfficiency, kWhAvail);
    printf("Hour\tPos\tNrg ct\tCons ct\tDiChg%%\tChgGri\tCon kWh\tRem kWh\n");
    for (int j = 0; j < HOUR_COUNT /*&& dischargeRate[consTariff[i].index] > 0*/; j++) {
        int i = (j + HOUR_COUNT + cfg->pvMinutes[month].end / 60) % HOUR_COUNT;
        int k = findOrigIndex(cfg, i);
        if (k >= 0) {
            if (cfg->dischargeRate[i] > 0) {
                if (cfg->consTariff[k].index <= cfg->pvMinutes[month].begin ||
                    cfg->consTariff[k].index >= cfg->pvMinutes[month].end) {
            ...
            printf("%3d\t%3d\t%7.3f\t%7.3f\t%2d\t%d\t%7.3f\t%7.3f\n",
                   ...
                   cfg->dischargeRate[cfg->consTariff[k].index],
                   cfg->chargeFromGrid[cfg->consTariff[k].index] ? 1 : 0,
...
int calcDischargeHours(config_t *cfg, int count, int month,
    ...
    while (i < count && cfg->consTariff[i].energyTariff > (cfg->dischargeThresholdNet) && kWh > 0) {
        ...
        printf("%3d\t%3d\t%7.3f\t%7.3f\t%2d\t%d\t%7.3f\t%7.3f\n",
               i, cfg->consTariff[i].index, cfg->consTariff[i].energyTariff, cfg->consTariff[i].consTariff,
               cfg->dischargeRate[cfg->consTariff[i].index],
               cfg->chargeFromGrid[cfg->consTariff[i].index] ? 1 : 0,
               cfg->consumption[month][cfg->consTariff[i].index],
               kWh);
...
int readConsumTariff(config_t *cfg, float gridTarff, int vatPerc) {
    ...
    while (readLine(file, text, TEXT_LEN) != NULL) {
        //fputs(text, stdout);
        // TODO: change to 15 o'Clock onwards
        if (strncmp("data_price_hour_abs_14_amount", text, 29) == 0) { // _abs_15_amount
...
int extractConsTariff(FILE *file, char txt[], int length, consTariff_t consTarff[], int count, float gridTarff,
    ...
    do {
        ...
        i++;
        // TODO: change to 15 o'Clock onwards -> ignore extra line only 9 times
        if (i < 10) { // < 9
