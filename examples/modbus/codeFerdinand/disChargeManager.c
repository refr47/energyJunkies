...
int main(int argc, char *argv[]) {
    ...
    readConsumTariff(&config, config.gridTariff, config.vatPercent);
    ...
    //minuteNow = calcMinute(timeNow); //config.pvMinutes[timeNow.tm_mon].end + 1; //calcMinuteNow(timeNow);
//    minutesToWait = config.pvMinutes[timeNow.tm_mon].end + 1 - minuteNow;
//    fprintf(stderr, "%s: sleeping for %d minutes from now", timeNowText, minutesToWait);
//    waitForMinutes(minutesToWait);
    ...
    // find most expensive hours and assign discharge rate to them until SoC below lower limit
    calcDischargeHours(&config, HOUR_COUNT, timeNow.tm_mon, inverterValues.kWhEnergy);
    fprintf(stdout, "\n[%s]\n", timeNowText);
    ...
    // TODO: fetch previous discharge rate from inverter
    ...
    do {
        ...
        if (timeNow.tm_hour != prevHour) {
            ...
            if (config.dischargeRate[timeNow.tm_hour] != prevDischrgRate) {
                fprintf(stderr, "%s: discharge rate (now: %d, prev.: %d)\n",
                        timeNowText, config.dischargeRate[timeNow.tm_hour], prevDischrgRate);
            ...
            if (config.chargeFromGrid[timeNow.tm_hour] != prevChargeFromGrid) {
                fprintf(stderr, "%s: charge from grid (now: %d, prev.: %d)\n",
                        timeNowText, config.chargeFromGrid[timeNow.tm_hour], prevChargeFromGrid);
...
//    writeInverter(inverter, (int) ((float) config.dischargePercent / inverterValues.chrgDisChrgSf), timeNowText,
//                  config.timeOffset);
...
// command line options handling
    ...
    opterr = 0;
    while ((cliArg = getopt(argc, argv, "i:p:c:t:o:x:d:f:s:lrh")) != -1) {
        switch (cliArg) {
            case 'i': // IP address
            ...
            case 'h':
                ...
                return 0;
            case '?':
                if (optopt == 'i' || optopt == 'p' || optopt == 'c' || optopt == 't' || optopt == '0' ||
                    optopt == 'x' || optopt == 'd' || optopt == 'f' || optopt == 's')
...
void showDischargeHours(config_t *cfg, int month, float kWhAvail) {
    bool doList;
    printf("VAT%%\tDiChg%%\tGrid ct\tFeed ct\tAdd ct\tRed ct\tDiCh ct\tChg ct\tBattEff\tAcc kWh\n%d\t%d\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\n",
    ...
    printf("Hour\tNrg ct\tCons ct\tDiChg%%\tChgGri\tCons kWh\tRem kWh\n");
    for (int i = 0, doList = false; i < HOUR_COUNT; i++, doList = false) {
        // show entries with discharge rate higher than 0
        if (cfg->dischargeRate[cfg->consTariff[i].index] > 0) {
            kWhAvail -= cfg->consumption[month][cfg->consTariff[i].index];
            doList = true;
        }
        // show one entry with discharge rate 0 after discharge rate higher 0
        if (i > 0 && cfg->dischargeRate[cfg->consTariff[i - 1].index] > 0) {
            doList = true;
        }
        // show entries with charge from grid set to true
        if (cfg->chargeFromGrid[cfg->consTariff[i].index] > 0) {
            doList = true;
        }
        if (doList) {
            printf("%02d\t%7.3f\t%7.3f\t%2d\t%d\t%7.3f\t%7.3f\n",
                   cfg->consTariff[i].index, cfg->consTariff[i].energyTariff, cfg->consTariff[i].consTariff,
                   cfg->dischargeRate[cfg->consTariff[i].index],
                   cfg->chargeFromGrid[cfg->consTariff[i].index] ? 1 : 0,
                   cfg->consumption[month][cfg->consTariff[i].index],
                   kWhAvail >= 0 ? kWhAvail : 0);
        }
    }
    fflush(stdout);
}
...
int calcDischargeHours(config_t *cfg, int count, int month,
                       float kWh) {
    int i = 0;
    while (i < count && cfg->consTariff[i].energyTariff > (cfg->dischargeThresholdNet) && kWh > 0) {
        kWh -= cfg->consumption[month][cfg->consTariff[i].index];
        // discharge only in most expensive hour or if SoC is high enough
        if (kWh >= 0 || i == 0) {
            cfg->dischargeRate[cfg->consTariff[i].index] = cfg->dischargePercent;
        }
        i++;
    }
    ...
    if (cfg->consTariff[count - 1].energyTariff < cfg->chargeThresholdNet) {
        cfg->chargeFromGrid[cfg->consTariff[count - 1].index] = 1;
    }
    return 0;
}

int readConsumTariff(config_t *cfg, float gridTarff, int vatPerc) {
    ...
    while (readLine(file, text, TEXT_LEN) != NULL) {
        //fputs(text, stdout);
        if (strncmp("data_price_hour_abs_14_amount", text, 29) == 0) {
            extractConsTariff(file, text, TEXT_LEN, cfg->consTariff, HOUR_COUNT, gridTarff, vatPerc);
...
int extractConsTariff(FILE *file, char txt[], int length, consTariff_t consTarff[], int count, float gridTarff,
    ...
    do {
        ...
        i++;
        if (i < 10) {
            readLine(file, txt, length); // ignore +xy/-xy entry between 14 and 23
        }
    } while (readLine(file, txt, length) == txt && i < count);
    if (i == count) {
        return 0;
...
// zusätzlicher, aktuell unbenutzer Quell-Code
