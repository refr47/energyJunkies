...
// modbus unit id of smart meter: 200 GEN24 @ home, 240 Symo @ HTL
    ...
    config_t config = {
            "10.0.0.13", 502, CUT_OFF_DURATION, CUT_OFF_BEGIN, CUT_OFF_BEGIN + CUT_OFF_DURATION,
...
// register values of smart meter
    int16_t meterRegs[METER_REGS_COUNT];
...
// register values of MPPT
    int16_t mpptRegs[MPPT_REGS_COUNT];
// register values of storage
    int16_t strgRegs[STRG_REGS_COUNT];
// register values of smart meter
    int16_t invrtRegs[INVRT_REGS_COUNT];
...
//fprintf(stderr, "%s:\n", "minute < config->cutOffBegin && values->socPerc >= config->...Thresh + 2");
        ...
        } else { // within cut off period => charge slowly
...
// 50% + (92% - 50%) * ((current - begin) / duration)
            ...
            if (!chrgPercSlowFactorCalculated) { // need to (re)calculate charge scale factor?
